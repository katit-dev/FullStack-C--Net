...
namespace backend_netcore_dotnet06.Controllers
{
    ...
    public class QuanLyNhanVienController : ControllerBase
    {
        ...
        public async Task<ActionResult> LayThongTinNhanVienTheoDuAn(int maDuAn)
        {
            ...
            foreach (var item in lstNhanVienTheoDuAn)
            {
                item.ConvertJsonNhanVienDuAn();
            }

            return Ok(lstNhanVienTheoDuAn);
        }

        // dto  cho procedure lay thong tin nhan vien theo du an
        ...
        // DTO cho store procedure lay thong tin nhan vien du an
        public class ChiTietDuAnDTO
        {
            public int MaDuAn { get; set; }
            public string TenDuAn { get; set; }
            public int SoNhanVien { get; set; }
            public string? DanhSachNhanVienJson { get; set; }
            ...
            {
                if (!string.IsNullOrEmpty(DanhSachNhanVienJson))
                {
                    DanhSachNhanVienChiTiet = JsonSerializer.Deserialize<List<TTNhanVienDuAnDTO>>(DanhSachNhanVienJson);
                }
            }
        }

        // DTO chi tiết nhân viên trong dự án
